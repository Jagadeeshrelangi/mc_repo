...
1. **Read `PROJECT_CONTEXT.md` first** — the v2.1 mental-model builder (the FIRST file an AI should read).
...
4. For deep detail, open the referenced folder by number:
   `02_architecture/diagrams/` (visuals), `03_database/`, `04_api/`, `05_navigation/`,
   `06_workflows/`, `07_modules/`, `02_architecture/glossary.md`.
...
### 2.4 Business model [src: BUSINESS_MODEL]
Revenue: 15–20% booking commission · fuel delivery margin ₹20–50 · seller
listing ₹99/mo · featured mechanic ₹499/mo · AI-diagnosis API ₹999/fleet/mo (B2B, Sprint 3).
Unit economics: CAC ₹150 · AOV ₹450 · LTV ₹2,250 · payback 3 transactions.
...
## 3. Architecture Knowledge [src: FRONTEND_ARCHITECTURE]
...
### 3.3 Provider Graph (`lib/app_wiring.dart`) — single source of truth
...
Non-provider singletons: `orderStore` (OrderStore), `ordersList` (in `lib/parts/order_data.dart`).
...
### 3.6 ID schemes (frozen) [src: API_CONTRACT]
...
All entities live in `lib/features/*/models/` + repository seed data. Seed facts:
...
## 5. API Knowledge [src: API_CONTRACT]
...
- Full method surface per module: see `04_api/`.

## 6. Navigation Knowledge [src: NAVIGATION_MAP]
...
## 7. Design System [src: UI_DESIGN_SYSTEM]
