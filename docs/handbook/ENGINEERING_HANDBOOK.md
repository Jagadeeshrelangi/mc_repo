...
**Classification** — Public. This document is the official open-source
technical reference for the Mecha Connect platform. Confidential business,
financial, security, and risk content is maintained in the internal handbook
and is excluded from this document.
...
6. [Chapter 6 — Product Requirements Document](#chapter-6-product-requirements-document)
7. [Chapter 7 — System Architecture](#chapter-7-system-architecture)
8. [Chapter 8 — Technology Stack](#chapter-8-technology-stack)
9. [Chapter 9 — Module Documentation](#chapter-9-module-documentation)
10. [Chapter 10 — User Workflows](#chapter-10-user-workflows)
11. [Chapter 11 — Navigation Documentation](#chapter-11-navigation-documentation)
12. [Chapter 12 — UI Design System](#chapter-12-ui-design-system)
13. [Chapter 13 — Database Blueprint](#chapter-13-database-blueprint)
14. [Chapter 14 — API Contract](#chapter-14-api-contract)
15. [Chapter 15 — Testing](#chapter-15-testing)
16. [Chapter 16 — Sprint History](#chapter-16-sprint-history)
17. [Chapter 17 — Deployment Roadmap](#chapter-17-deployment-roadmap)
18. [Chapter 18 — Future Scope](#chapter-18-future-scope)
19. [Chapter 19 — Known Limitations](#chapter-19-known-limitations)
20. [Chapter 20 — Appendix](#chapter-20-appendix)
...
rather than a bundle of documents, so an engineer, reviewer, or
...
### 4.3 The Need for Mecha Connect
...
## Chapter 6 — Product Requirements Document
...
and the seeded `ordersList` in `frontend/lib/parts/order_data.dart`; `addMarketplaceOrder`
...
## Chapter 9 — Module Documentation

### 10.1 Splash, Onboarding and Authentication

- **Splash** (`frontend/lib/main.dart`): timed branded sequence with glow/scale/rotation
...
## Chapter 11 — Navigation Documentation
...
## Chapter 12 — UI Design System

### 13.1 Colors (`frontend/lib/theme/app_colors.dart`)
...
| File | Count | Coverage |
|---|---|---|
| `frontend/test/ai_module_test.dart` | 25 | AI home, chat, diagnosis, history, pin/refresh merge, retry/failure |
| `frontend/test/fuel_module_test.dart` | 37 | stations, booking, price, lifecycle, tracking, invoice, history, failure |
| `frontend/test/marketplace_module_test.dart` | 43 | catalog, categories, cart, checkout, orders, wishlist, coupons, responsive |
| `frontend/test/profile_module_test.dart` | 30 | profile, vehicles, addresses, wallet, rewards, settings, order feed |
| `frontend/test/mechanic_module_test.dart` | 10 | mechanic list, details, booking, AI vehicle form |
| `frontend/test/vehicle_location_test.dart` | 8 | vehicle location flow |
| `frontend/test/home_dashboard_test.dart` | 3 | dashboard render + sections |
| `frontend/test/integration/runtime_marketplace_flow_test.dart` | 2 | end-to-end flow on the production graph |
| `frontend/test/widget_test.dart` | 4 | rating semantics, touch targets + tooltips, quick-services grid |
| **Total** | **162** | |
...
v1 prototype (legacy `frontend/lib/Auth`, `frontend/lib/home`, `frontend/lib/mechanic`, per-screen mock
data, legacy HTTP seams) evolved into the v2 feature-first architecture
(`frontend/lib/features/*` with models → repositories → providers → screens →
...
| Enterprise | B2B AI diagnosis API (fleet subscription) |
...
## Chapter 19 — Known Limitations

### 19.1 Known Limitations (accepted at RC1)
...
- **P3 defensive:** `loadReviews` / `cancelActiveBooking` /
  `completeActiveBooking` lack try/catch (mock-acceptable at RC1).
...
shared_preferences, flutter_lints (dev), flutter_test (dev). See `frontend/pubspec.yaml`.

### 20.3 Licenses

All packages are used under their respective open-source licenses
(predominantly BSD-3-Clause and MIT). This document is public and may be
redistributed under the project license. Full third-party license texts are
recorded in the project's dependency license notes.
...
- `FRONTEND_LOCK_REPORT.md` — freeze governance (archive/sprint_history)
- `QA_CERTIFICATION_REPORT.md` — certification evidence (archive/sprint_history)
- `PROJECT_STATUS_REPORT.md` — status (archive/sprint_history)
- `common/ARCHITECTURE_OVERVIEW.md` — high-level system view
- `frontend/Architecture.md` — provider graph, module tree, seams
- `frontend/Design_System.md` — frozen design tokens
- `frontend/Navigation.md` — full navigation maps
- `backend/API.md` — Sprint 2 API contract
- `backend/Database.md` — Sprint 2 schema
- `common/*` — PRD, feature specs, roadmap, project status, changelog,
  contributing, coding standards, glossary, installation
- `archive/*` — superseded historical documents (sprint_history, legacy, source)

### 20.6 Version History

| Version | Date | Change |
|---|---|---|
| 1.0.1 | 2026-08-06 | Public/private split — Chapter 6 (Business Model), competitive analysis, and the risk register moved to the internal handbook; public handbook renumbered 1–20 |
