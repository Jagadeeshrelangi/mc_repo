...
**Version:** 1.9.4 (Documentation Restructure)  
...
## [1.9.4-restructure] — 2026-08-06 — Documentation Public/Private Restructure

### Documentation
- **Canonical `docs/` structure finalized** — `common/` (8 canonical files +
  supporting), `frontend/` (11 canonical files + supporting), `backend/`
  (9 canonical files + supporting), `handbook/` (`ENGINEERING_HANDBOOK.md` +
  `CHANGELOG.md`). See `docs/CANONICAL_DOCUMENT_MAP.md`.
- **Handbook renamed** to `ENGINEERING_HANDBOOK.md` and **sanitized for public
  release** — Chapter 6 (Business Model), competitor analysis, and the risk
  register removed from the public edition; renumbered 1–20; appendix and
  references updated.
- **Confidential documentation set created** — confidential content (business
  model, financials, risk register, investor notes, security, operations,
  internal roadmap, decisions, internal handbook) separated out of the public
  tree; excluded from the
  repository via `.gitignore`.
- **`CHANGELOG.md` moved** to `docs/handbook/` (release history is part of the
  handbook).
- **`SECURITY.md` added** at repo root — responsible disclosure, no-secrets rule.
- **Stale cross-references repaired** across moved canonical docs (frontend
  `Architecture.md`, `Design_System.md`, backend `Deployment.md`, `Database.md`,
  `ROADMAP_PUBLIC.md`, `PROJECT_OVERVIEW.md`, `DEVELOPMENT_WORKFLOW.md`).
- **Synced:** `docs/README.md`, `CANONICAL_DOCUMENT_MAP.md`, root `README.md`.

---

## [1.9.3-cleanup] — 2026-08-06 — Pre-Sprint 2 Engineering Cleanup

### Security
- **Plaintext password persistence removed** (`auth_provider.dart`): `login()`/`register()`
  no longer store `remember_me_password`; `_loadSavedCredentials()` reads email only.
- **Login auto-fill of saved password removed** (`login_screen.dart`) — pre-fills email only.
- **`logout()` always clears stored credentials** regardless of "remember me".
- **Mock auth seam documented** (`auth_repository.dart`): explicit dev-only mock comment.

### Documentation
- **8 broken links repaired** (archived `05_reports/` paths) — full-repo scan: 0 broken links.
- **6 leaked tool-artifact XML blocks removed** (AI_PROJECT_MEMORY, PROJECT_TIMELINE,
  GAP_ANALYSIS, SPRINT_2_BACKEND_BLUEPRINT, BACKEND_AUDIT_REPORT, BACKEND_BLUEPRINT).
- **`PROJECT_TIMELINE.md` rewritten** — fabricated 12-month narrative replaced with
  canonical 2026-07-20 → 2026-08-05 history (matches CHANGELOG + git).
- **False "validation PASS" claims corrected** in archived refactor/completion reports.
- **Synced:** `PROJECT_STATUS.md` (v1.3.0), `ROADMAP.md` (v1.2.0), `docs/README.md`,
  `NEXT_SESSION_HANDOVER.md`, `CANONICAL_DOCUMENT_MAP.md`.
- **Deliverable:** `ENGINEERING_REVIEW_REPORT.md` (repo root) — Sprint 2 baseline + debt register.

### Cleanup
- **Flutter deps removed:** `flutter_map_tile_caching`, `flutter_map_cancellable_tile_provider`,
  `cupertino_icons` (0 imports found).
- **`requirements.txt` pinned** to venv freeze (no `>=`).
- **Backend settings centralized:** CORS allow-list (`settings.CORS_ORIGINS`),
  `GEMINI_MODEL`, `DEFAULT_VEHICLE_MILEAGE` single-source in `config.py`;
  `ENABLE_FALLBACK` default flipped `False → True` (resilient dev default; `.env` still pins `False`).
- **`.env.example` added** (root + `backend/`); `.gitignore` hardened
  (`!.env.example` exceptions, `devtools_options.yaml`, `Thumbs.db`, `desktop.ini`).
- `backend/app/services/chat_service.py` — unused `Any` import removed.
- `__pycache__/` + venv cache directories cleaned; no secrets found in tracked source.

### Verified
- `flutter analyze` — **0 issues**
- `flutter test` — **162/162 passing**
- Backend imports load — **6 routes** registered; all 6 JSON exports valid.

---

## [1.9.3-docs] — 2026-08-06 — Independent engineering review

### Added
- **Independent engineering audit** (read-only) of the full repo: frontend,
  backend, AI pipeline, docs integrity, timeline, security, testing.
  Report archived internally (not part of the public documentation tree).
- `INSTALLATION.md` §5 "Backend Scaffold — As-Built" — verified backend scope
  (28 tracked files, 18 `.py`, 6 endpoints), config defaults
  (`ENABLE_FALLBACK=false`, `GEMINI_API_KEY=None`), known constraints (no
  DB/auth/tests/Alembic, CORS wildcard, unpinned requirements, hardcoded
  `mileage=80000`, in-memory session store, FAISS dangerous-deserialization
  flag) and AI asset inventory.
- `INSTALLATION.md` §1D corrected — `.env` keys now match `config.py`
  (`ENABLE_FALLBACK` added; no `DATABASE_URL` until a DB layer exists).

### Verified (re-run during audit, working tree)
- `flutter analyze` — **0 issues**
- `flutter test` — **162/162 passing**

### Notes
- Frontend architecture claims (9 root providers, 5-tab shell, 7 modules,
  7 repositories) re-verified **TRUE** — no doc changes required.
- Timeline (2026-07-20 → 2026-08-05) re-verified against `CHANGELOG.md` + git;
  the `docs/common/PROJECT_TIMELINE.md` 12-month narrative
  contradicts it and is flagged for correction.
...
### Changed
- `frontend/pubspec.yaml` version bumped to **`1.0.0+1`** (RC1 release build)
...
- Handbook rendered to **PDF (39 pages)** and **DOCX (21 chapters + TOC field)**
  from the Markdown source (public edition now `ENGINEERING_HANDBOOK.md`; the
  historical PDF/DOCX with private chapters are retained as confidential renders)
...
- `MECHA_CONNECT_MASTER_HANDBOOK.md` §20 records audit outcome — intentional
  ...
  (public edition: `ENGINEERING_HANDBOOK.md` §19)
...
### Added
- `MECHA_CONNECT_MASTER_HANDBOOK.md` (v2.0.0) — draft handbook; superseded by
  `ENGINEERING_HANDBOOK.md` (v1.0.0/1.0.1), which itself supersedes
  `docs/archive/MASTER_ENGINEERING_HANDBOOK_v1.0.md`
...
- **Code quality:** `frontend/lib/debug/runtime_trace.dart` deleted + all wiring/imports stripped; `forceShowOnboarding` dev flag removed; orphan `coming_soon.dart` deleted; brace-wrapped 3 pre-existing `if` infos

### Removed
- `frontend/lib/debug/` (runtime trace), `frontend/lib/features/marketplace/widgets/coming_soon.dart`, `MyApp(navigatorObservers:)` param, `forceShowOnboarding` flag

### Changed
- `frontend/test/integration/runtime_marketplace_flow_test.dart` uses a local `_TestNavigatorObserver` (same hash/push/pop tracking)
...
- `OrderStore` singleton (`frontend/lib/parts/order_data.dart`) so the Orders tab (mounted in an `IndexedStack`) rebuilds after Marketplace order inserts

### Fixed
- **P0** Vehicle form no longer calls legacy real HTTP to `127.0.0.1:8000` (≈90s hang) — now uses the AI mock engine via `DiagnosisService` (`frontend/lib/features/mechanic/screens/vehicle_form_screen.dart`)
- **P0** AI provider shared a single `AiRepository` across provider + AiService + DiagnosisService; refresh preserves user conversations and pin overrides (`frontend/lib/features/ai/providers/ai_provider.dart`)
- **P0** Orders tab stale after Marketplace checkout — shell uses `IndexedStack` + `OrderStore` notify; removed rebuild-every-tick tab listener (`frontend/lib/bottom_bar/bottom_navigation.dart`, `frontend/lib/bottom_bar/order_screen.dart`)
...
- `kRuntimeTrace = false` (`frontend/lib/debug/runtime_trace.dart`)

### Removed
- 29 dead/legacy files — `frontend/lib/services/ai_repository.dart`, `frontend/lib/services/api_client.dart`, `frontend/lib/bottom_bar/chatboard.dart`, 12 orphaned widgets, unused module barrels, dead auth model, legacy verify scripts
- Restored/fixed `frontend/lib/features/fuel_delivery/widgets/widgets.dart` barrel (now exports 12 widgets)

### Changed
- `MechanicProvider` accepts an optional `MechanicRepository` (constructor injectable)
- `frontend/test/integration/runtime_marketplace_flow_test.dart` uses `.first` for `'Parts'` taps (IndexedStack keeps hidden tab labels in the tree)
...
### Added
- Fuel Delivery feature foundation (`frontend/lib/features/fuel_delivery/`)
...
### Changed
- `frontend/lib/Starting_screen/` → `frontend/lib/starting_screen/`, `Login.dart` → `login.dart`
- `frontend/lib/home/mock_data.dart` → `frontend/lib/home/home_data.dart`
...
### Removed
- `frontend/lib/screens/mechanic_map_screen.dart` (unreachable, superseded by MechanicHomeScreen)
- `frontend/lib/widgets/mechanic_card.dart` (duplicate of `frontend/lib/mechanic/widgets/mechanic_card.dart`)
- `frontend/lib/widgets/status_chip.dart` (dead code)
...
- frontend/pubspec.yaml configuration
