...
> Sprint 1.9b · Frozen design tokens · Source of truth: `frontend/lib/theme/`

All tokens below are frozen for RC1. No color/typography/spacing changes
without sprint sign-off (see `archive/sprint_history/FRONTEND_LOCK_REPORT.md`).

## 1. Brand Colors (`frontend/lib/theme/app_colors.dart`)
...
## 2. Spacing (`frontend/lib/theme/app_spacing.dart`)
...
## 5. Responsive System (`frontend/lib/theme/app_responsive.dart`)
...
- `ThemeProvider` (`frontend/lib/theme/theme_provider.dart`): `ThemeMode.system` default;
...
- **Loading / Empty / Error:** dedicated state widgets per module; loaders
  ...
  `frontend/lib/features/*/widgets/` and `frontend/lib/widgets/`); error states offer retry and
