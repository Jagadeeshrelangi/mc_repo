...
> Flutter 3.29.2 · Provider (6.x) · nested · google_nav_bar · device_preview
...
## 2. App Entry (`frontend/lib/main.dart`)
...
## 3. Provider Graph (`frontend/lib/app_wiring.dart`)
...
- `orderStore` (`frontend/lib/parts/order_data.dart`) — `OrderStore extends ChangeNotifier`; the
...
`BottomNavigation` (`frontend/lib/bottom_bar/bottom_navigation.dart`):
...
only on the repository interfaces (see `../backend/API.md`).
