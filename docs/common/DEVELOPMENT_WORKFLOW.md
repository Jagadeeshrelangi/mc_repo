# DEVELOPMENT WORKFLOW — Mecha Connect
...
1. **`docs/` is canonical** — one source of truth per topic (see CANONICAL_DOCUMENT_MAP.md).
...
4. **Entry order for AI:** PROJECT_OVERVIEW → KNOWLEDGE_GRAPH → MASTER_PROJECT_KNOWLEDGE_BASE → MASTER_PROJECT_DATA.json → DEVELOPMENT_WORKFLOW.
...
## 8. Known Issues (engineering)

Security findings, secrets, and vulnerability details are tracked internally and
never listed in public docs.
