# PROJECT_OVERVIEW — Mecha Connect

> **Documentation Restructure v1.0 · 2026-08-06**
...
- Feature-first modules under `frontend/lib/features/`: ai, auth, home, marketplace, mechanic,
...
## 6. Folder Structure (frontend/lib/)
...
- Canonical tree: `docs/` — active docs in `common` (project/product/dev),
  `frontend` (architecture, ui, modules, navigation, workflows, testing, assets),
  `backend` (architecture, api, database, deployment); engineering handbook in
  `handbook/`; machine-readable indexes in `exports/`; historical records in
  `archive/` (sprint_history, legacy, source).
- **Engineering Handbook:** 20-chapter book (md) — the canonical public narrative.
- **Doc build:** consolidated `docs/` — one canonical doc per topic,
  full archive taxonomy (sprint_history / legacy / source).
