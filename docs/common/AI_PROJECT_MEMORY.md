...
| Artifact | Location |
|---|---|
| Regenerated knowledge_graph.json (v2.1 structure) | `exports/knowledge_graph.json` |
| Updated MASTER_PROJECT_DATA.json | `exports/MASTER_PROJECT_DATA.json` |
| Updated MASTER_PROJECT_KNOWLEDGE_BASE.md | `common/MASTER_PROJECT_KNOWLEDGE_BASE.md` |
| AI_PROJECT_MEMORY.md | `common/` (this file) |
| PROJECT_TIMELINE.md | `common/` |
| PROJECT_OPERATING_MANUAL.md | `common/DEVELOPMENT_WORKFLOW.md` (renamed) |

## 4. Key Decisions Made

1. **Audit approved** — Phase 0 Engineering Audit is the official Sprint 2 baseline.
2. **Handbook is Version 1** — `handbook/ENGINEERING_HANDBOOK.md` is the public canonical handbook; v2.1 only prepares enrichment, does NOT regenerate.
3. **Single documentation tree** — `docs/` is canonical; legacy `docs/` and `documentation_build/` were fully consolidated into it.
4. **No git operations** — Phase 7 blocked until explicit approval.
5. **No backend work** — Sprint 2 blocked until Phase 7 approval.
6. **Entry order for AI:** PROJECT_OVERVIEW → KNOWLEDGE_GRAPH → MASTER_PROJECT_KNOWLEDGE_BASE → MASTER_PROJECT_DATA.json → DEVELOPMENT_WORKFLOW.
...
- Do NOT modify anything outside `docs/` (application code stays frozen).
...
2. Read `PROJECT_OVERVIEW.md` → `KNOWLEDGE_GRAPH.md` → `MASTER_PROJECT_KNOWLEDGE_BASE.md`.
3. Read `CANONICAL_DOCUMENT_MAP.md` for the single source of truth per topic.
4. Read `DEVELOPMENT_WORKFLOW.md` for build, test, and architecture rules.
